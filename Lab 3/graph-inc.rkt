...
;; Make a new graph
(define makeGraph
  (lambda (author type shape color)
    (list author type shape color)))

;; author -- selector for the type of a graphical unit
(define author
  (lambda (graph)
    (car graph)))
...
(define shape
  (lambda (graph)
    (caddr graph)))
;; color -- selector for the type of a graphical unit
(define color
  (lambda (graph)
    (cadddr graph)))
...
(define makePoint
  (lambda (x y)
    (makeGraph 'Graph-Inc 'point (list x y) 'blue)))
...
(define makeCircle
  (lambda (x0 y0 radius)
    (makeGraph 'Graph-Inc 'circle (list (makePoint x0 y0) radius )'yellow)))

;; makeLine -- constructor for 'line_segment
(define makeLineSegment
  (lambda (x0 y0 x1 y1)
    (makeGraph 'Graph-Inc 'line_segment
...
(define makeRectangle
  (lambda (x0 y0 x1 y1)
    (makeGraph 'Graph-Inc 'rectangle
               (list (makePoint (min x0 x1) (max y0 y1))
                     (makePoint (max x0 x1) (min y0 y1))) 'green)))
               

;; makePict -- constructor for 'pict
(define makePict
  (lambda (picts)
    (makeGraph 'Graph-Inc 'pict picts 'black)))
