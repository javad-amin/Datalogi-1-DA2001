...
;; make-set-from-elems -- procedure for creating a new set - A new variant
(define make-set-from-elems
  (lambda rest-id
    ...
    (make-object 'set (del-dup (reverse (iter rest-id '()))))))
...
(define difference
  (lambda (set1 set2)
        (cond
          ((empty? set2) set1)
          ((member? (first set2) set1) (difference (set-delete (first set2) set1) (rest set2)))
