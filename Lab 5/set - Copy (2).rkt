  ...
  (lambda (lst)
    ...
    (define iter
      (lambda (lst result)
        (cond
          ((empty? lst) result)
          ((set? lst) (iter (rest lst) result))
          ((list? (first lst))
           (if (member? (make-set (first lst)) (make-set (rest lst)))
               (iter (rest lst) result)
               (iter (rest lst) (insert (make-set (first lst)) result))))
          (else
           (if (member? (first lst) (rest lst))
               (iter (rest lst) result)
               (iter (rest lst) (insert (first lst) result)))))))
    
    (make-object 'set (reverse (iter lst '())))))
...
(define subset?
  (lambda (set1 set2)
    (define iter
      (lambda (set1 set2 counter)
        (cond
          ((= counter 0) #t)
          ((member? (first set1) set2) (iter (rest set1) set2 (- counter 1)))
          (else #f))))
